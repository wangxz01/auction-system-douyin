...
func ListAdminOrders(c *gin.Context) {
	...
	query := config.DB.Table("orders").Select("orders.*").Joins("JOIN auctions ON auctions.id = orders.auction_id")
	if !isAdminUsername(username) {
		if !isActiveMerchant(uid) {
			c.JSON(http.StatusForbidden, gin.H{"error": "需要商家权限"})
			return
		}
		query = query.Where("auctions.seller_user_id = ?", uid)
	}
	...
	if len(orders) == 0 {
		c.JSON(http.StatusOK, gin.H{"data": []AdminOrderEntry{}})
		return
	}
