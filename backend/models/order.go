...
type Order struct {
	...
	FinalPriceCents int64     `gorm:"not null;default:0" json:"final_price_cents"`
...
func CreateOrderForAuction(db *gorm.DB, auctionID, userID uint, finalPrice float64) error {
	o := Order{
		...
		FinalPriceCents: int64(finalPrice*100 + 0.5),
		Status:          "pending",
	}
	return db.Create(&o).Error
}

func CreateOrderForAuctionCents(db *gorm.DB, auctionID, userID uint, finalPriceCents int64) error {
	o := Order{
		AuctionID:       auctionID,
		UserID:          userID,
