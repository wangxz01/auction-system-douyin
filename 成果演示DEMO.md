...
6. **可证明工程材料**：提供演示脚本、方案文档、AI 使用说明、k6 压测脚本、真实压测结果和测试覆盖。

## 5. 端到端使用流程

1. 管理员或商家登录后台，进入发布商品页面。
2. 商家上传商品图片，填写名称、介绍、直播地址，并配置起拍价、加价幅度、封顶价、竞拍时长和自动延时规则。
3. 商家在竞拍管理页点击“开始”，系统将竞拍状态从 `pending` 切换到 `active` 并广播给直播间用户。
4. 用户进入移动端直播间，查看直播画面、商品信息、倒计时、当前价和实时排行榜。
5. 多个用户同时出价时，后端通过 Redis 锁、MySQL 行锁和幂等键保证最高价、赢家和排行榜一致。
6. 用户被超越、领先、自动延时或竞拍结束时，前端通过 WebSocket 实时展示动画、提示和音效。
7. 出价达到封顶价或倒计时结束后，系统自动结束竞拍并生成唯一订单。
8. 中标用户进入订单页模拟支付，商家在后台订单管理中查看成交结果。
...
1. 管理员登录并发布竞拍商品。
2. 配置 0 元起拍、加价幅度、封顶价、自动延时。
3. 开始竞拍。
4. 两个用户端窗口同时出价。
5. 展示排行榜变化、领先/被超越提示、自动延时。
6. 触发封顶成交或倒计时结束。
7. 中标用户查看订单，商家后台查看成交订单。
